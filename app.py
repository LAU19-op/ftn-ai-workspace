...
def inicializar_bd():
    if "proyectos" not in st.session_state:
        ...
        # REPARACIÓN DE ESTRUCTURAS FALTANTES (Evita el error 734)
...
# (Acá continúan los demás modales de proyecto que ya tenías, no quité ni uno)
            ...
            with st.container(border=True):
                ...
                if st.button("Registrar", use_container_width=True):
                    if n_reg and e_reg and p_reg and f_reg:
                        db[e_reg] = {"nombre": n_reg, "pass": p_reg, "rol": "Invitado", "nivel": "lectura", "estado": "Pendiente", "foto": base64.b64encode(f_reg.read()).decode('utf-8'), "credencial": f"FTN-{random.randint(1000,9999)}", "edad": "", "roles_fav": "", "portfolio": "", "spotify": "", "amigos": []}
                        guardar_y_recargar()
...
# --- 7. PLATAFORMA ---
    ...
    with c_nav6:
        # Foto y botón en el mismo contenedor para alineación exacta
    ...
    if st.session_state["ruta"] == "Inicio":
        ...
        with c_main:
            ...
            if rol_actual == "Super Admin":
                ...
                cc1.metric("CPU", f"{psutil.cpu_percent()}%")
                cc2.metric("RAM", f"{psutil.virtual_memory().percent}%")
